...
import 'package:zporter_board/features/tacticV2/presentation/view/component/r&d/tactic_board_game.dart';
...
class PlayerSpriteComponent extends SpriteComponent
  ...
  void onDragUpdate(DragUpdateEvent event) {
    ...
    if (!_isRotationHandleDragged) { // Prevent drag if rotation handle is dragged
      position += event.canvasDelta;
      debug(data: "Drag callback event Player ${event.localDelta}");
    }
  }
...
class SelectionBorder extends RectangleComponent {
  ...
  SelectionBorder({required this.player})
    ...
    add(ScalingHandle(player: player, anchor: Anchor.topLeft, color: Colors.white)
      ..position = topLeft);
    add(ScalingHandle(player: player, anchor: Anchor.topRight, color: Colors.red)
      ..position = topRight);
    add(ScalingHandle(player: player, anchor: Anchor.bottomLeft, color: Colors.black)
      ..position = bottomLeft);
    add(ScalingHandle(player: player, anchor: Anchor.bottomRight, color: Colors.blue)
  ...
  void updateHandlePositions() {
    ...
    children.whereType<RotationHandle>().first.position.setFrom(Vector2(size.x / 2, 0));
  }
}



class RotationHandle extends PositionComponent { // Remove DragCallbacks
...
class ScalingHandle extends CircleComponent with DragCallbacks {
  ...
  ScalingHandle({required this.player, required this.anchor, this.color = const Color(0xFF00FF00)})
      : super(
    radius: 6,
  ...
  @override
  void onDragUpdate(DragUpdateEvent event) {
    debug(data: "Dragging the scale ${event.localDelta}");
    Vector2 delta = event.localDelta;
    ...
    player._selectionBorder?.position.setFrom(Vector2(player.size.x / 2, player.size.y / 2));
