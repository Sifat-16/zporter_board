...
class GameField extends PositionComponent with HasGameReference<TacticBoardGame> {
  ...
  final Paint _fieldPaint = Paint()..color = const Color(0xFF4CAF50); // Green Field
  ...
  void _drawPenaltySpots(Canvas canvas) {
    // Left Penalty Spot
    canvas.drawCircle(
      Offset(penaltyBoxWidth-(penaltyBoxWidth - goalBoxWidth)/2, size.y / 2),
      penaltySpotRadius,
      _borderPaint,
    );
    ...
    // Right Penalty Spot
    canvas.drawCircle(
      Offset(size.x - penaltyBoxWidth + (penaltyBoxWidth - goalBoxWidth)/2, size.y / 2),
      penaltySpotRadius,
      _borderPaint,
    );
  }
