...
class MatchDataSourceImpl implements MatchDataSource {
  final FirebaseFirestore firestore;
  final FirebaseAuth firebaseAuth;
  final AuthBloc authBloc; // MODIFIED: Replaced UserIdService

  MatchDataSourceImpl({
    required this.firestore,
    required this.firebaseAuth,
    required this.authBloc, // MODIFIED: Replaced UserIdService
  });

  /// Gets the current Firebase User ID from the AuthBloc.
  String _getCurrentUserId() {
    // MODIFIED: Gets the user ID from the central AuthBloc state.
    final user = authBloc.state.user;
    if (user.uid.isEmpty) {
      throw Exception('User not authenticated or UID is empty');
    }
    return user.uid;
  }
  ...
  DocumentReference _getUserMatchDocRef() {
    final userId = _getCurrentUserId();
    // Assuming FirestoreConstants.matches is your collection name, e.g., 'matches'
    return firestore.collection(FirestoreConstants.matches).doc(userId);
  }
  // --- End Helper ---
  ...
  Future<int> clearMatchDb() async {
    ...
    try {
      await deleteMatch(_getCurrentUserId());
      return 1;
    } catch (e) {
      debug(data: "Firebase: Error during clearMatchDb (delete): $e");
      return 0;
