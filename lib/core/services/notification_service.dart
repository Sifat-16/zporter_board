...
import 'package:path_provider/path_provider.dart'; // For finding a temporary path
import 'package:zporter_board/core/services/injection_container.dart';
import 'package:zporter_board/features/notification/domain/repository/notification_repository.dart';
import 'package:zporter_board/features/notification/presentation/view_model/notification_bloc.dart';
import 'package:zporter_board/features/notification/presentation/view_model/notification_event.dart';
import 'package:zporter_board/features/notification/presentation/view_model/unread_count_bloc.dart';
import 'package:zporter_board/features/notification/presentation/view_model/unread_count_event.dart';
import 'package:zporter_board/main.dart'; // Import main.dart to get the helper function
import 'package:zporter_tactical_board/app/helper/logger.dart';
...
class NotificationService {
  ...
  // A helper method to show a rich notification with an image.
  Future<void> _showBigPictureNotification(
      RemoteNotification notification, String imageUrl) async {
    try {
      final String tempPath = (await getTemporaryDirectory()).path;
      final String filePath = '$tempPath/notification_image.jpg';
      ...
      // Fallback to a simple notification if image download fails.
      _showSimpleNotification(notification);
    }
  }

  // A fallback method for showing simple text-only notifications.
  ...
  void _configureMessageListeners() {
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      print("Foreground Message Received: ${message.notification?.title}");
      _saveNotification(message); // Save the notification with all its data.
      sl<UnreadCountBloc>().add(IncrementUnreadCount());

      RemoteNotification? notification = message.notification;
      if (notification == null) return;

      // Check for a cover image URL and show the appropriate notification.
      final coverImageUrl = defaultTargetPlatform == TargetPlatform.android
          ? notification.android?.imageUrl
          : notification.apple?.imageUrl;

      if (coverImageUrl != null) {
        _showBigPictureNotification(notification, coverImageUrl);
      } else {
        _showSimpleNotification(notification);
      }
    });
    ...
    FirebaseMessaging.onMessageOpenedApp.listen((RemoteMessage message) {
      ...
      // This will be enhanced later to navigate to a detail screen.
  ...
  // Uses the shared parser from main.dart to create the notification model.
  Future<void> _saveNotification(RemoteMessage message) async {
